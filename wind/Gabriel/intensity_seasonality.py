...
OMEGA_YEAR = 2.0 * np.pi / (365.0 * 24.0)
...
def notebook_calendar_design(index: pd.DatetimeIndex, *, fourier_order: int = 2) -> pd.DataFrame:
    ...
    for k in range(1, int(fourier_order) + 1):
        angle = 2.0 * np.pi * float(k) * df["doy"] / 365.0
        df[f"sin{k}"] = np.sin(angle)
        df[f"cos{k}"] = np.cos(angle)
    X_dummies = pd.get_dummies(df[["hour", "weekday", "month"]], drop_first=True, dtype=float)
    fourier_cols = [f"sin{k}" for k in range(1, int(fourier_order) + 1)] + [f"cos{k}" for k in range(1, int(fourier_order) + 1)]
...
def harmonic_parameter_table(fit: SeasonalityFit) -> pd.DataFrame:
    ...
    h = fit.harmonic_params
    return pd.DataFrame(
        [
            {"parameter": "beta0", "value": h.intercept},
            {"parameter": "beta1", "value": h.trend},
            {"parameter": "beta2", "value": h.quad},
            {"parameter": "beta3", "value": h.year_cos},
            {"parameter": "beta4", "value": h.year_sin},
            {"parameter": "beta5", "value": h.week_cos},
            {"parameter": "beta6", "value": h.week_sin},
            {"parameter": "beta7", "value": h.day_cos},
            {"parameter": "beta8", "value": h.day_sin},
        ]
    )


def notebook_parameter_table(fit: SeasonalityFit) -> pd.DataFrame:
    if fit.parameterization not in {"mle_notebook", "solar_logit"}:
        raise ValueError("Notebook parameter table requested for a non-notebook-style seasonality fit.")
