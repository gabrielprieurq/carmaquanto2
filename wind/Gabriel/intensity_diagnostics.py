...
def joint_dependence_summary(
    ...
    simulated_wind: np.ndarray,
) -> pd.DataFrame:
    emp = empirical.dropna()
    sim_spot = flatten_simulation_paths(simulated_spot, max_points=min(simulated_spot.size, 20000))
    sim_wind = flatten_simulation_paths(simulated_wind, max_points=min(simulated_wind.size, 20000))
    n = min(len(sim_spot), len(sim_wind))
    sim_spot = sim_spot[:n]
    sim_wind = sim_wind[:n]
    return pd.DataFrame(
        [
            {
                ...
                "simulated": float(np.corrcoef(sim_spot, sim_wind)[0, 1]),
            },
            {
                "measure": "spot_mean_low_wind",
                "empirical": float(emp.loc[emp.iloc[:, 1] <= emp.iloc[:, 1].median(), emp.columns[0]].mean()),
                "simulated": float(np.mean(sim_spot[sim_wind <= np.median(sim_wind)])),
            },
            {
                "measure": "spot_mean_high_wind",
                "empirical": float(emp.loc[emp.iloc[:, 1] > emp.iloc[:, 1].median(), emp.columns[0]].mean()),
                "simulated": float(np.mean(sim_spot[sim_wind > np.median(sim_wind)])),
            },
        ]
