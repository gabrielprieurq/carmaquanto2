...
try:
    from .intensity_calibration_solar import IntensityCalibrationWorkspace
    from .intensity_model_solar import IntensityModelSpecification
    from .intensity_model_stochastic_covariance_solar import (
    ...
    from .intensity_wishart_stochastic_covariance_solar import (
...
def select_base_model_spec(
    ...
    *,
    ...
    if variant not in workspace.model_specs:
        raise ValueError(f"Unsupported variant: {variant}")
    return workspace.model_specs[variant]
...
def _select_variants_for_index(
    ...
    if extension.base_variant == "solar_clearsky":
        return (
            extension.base_workspace.spot_variants["paper"],
            extension.base_workspace.solar_variants["solar_clearsky"],
        )
    return (
        extension.base_workspace.spot_variants["mle_notebook_full"],
        extension.base_workspace.solar_variants["mle_notebook_raw"],
    )
...
def run_stochastic_covariance_extension(
    ...
    *,
    ...
    reference_continuous_wishart = fit_wishart_full_covariance_model(
        state_pair.rename(columns={"spot_state": "spot_innovation", "solar_state": "solar_innovation"}),
    ...
    stochastic_spec = build_stochastic_covariance_spec(base_spec, companion_fit, wishart, name=f"{variant}_stochastic_covariance")
...
def simulate_constant_vs_stochastic_covariance(
    ...
    *,
    ...
    stochastic_spec, compare_index = _build_rebased_spec_for_comparison(extension, index=pd.DatetimeIndex(index))
    baseline = simulate_intensity_paths_constant_covariance(stochastic_spec, index=compare_index, num_paths=num_paths, seed=seed)
