...
def fit_var24_companion(
    ...
    *,
    ...
    pair = pair.iloc[:, :2]
    pair.columns = ["spot_state", "solar_state"]
    ...
    coefs = np.stack([beta[2 * lag : 2 * (lag + 1), :].T for lag in range(p)], axis=0)
    ...
    resid_df = pd.DataFrame(residual, index=pair.index[p:], columns=["spot_innovation", "solar_innovation"])
    fitted_df = pd.DataFrame(fitted, index=pair.index[p:], columns=["spot_fitted", "solar_fitted"])
...
def _simulate_companion_core(
    ...
    *,
    ...
    solar_latent_season = _evaluate_seasonality(base.solar_seasonality, idx)
    solar_clear = evaluate_clear_sky_proxy(base.solar_seasonality, idx)
    ...
    solar_latent0 = solar_latent_season[0] + states[:, 0, 1]
    solar_cf[:, 0] = _latent_to_physical(base.solar_seasonality, np.full(P, solar_clear[0], dtype=float), solar_latent0)
    ...
    solar_thr = float(base.intensity.negative_two_state.renewable_threshold)
    lambda_neg[:, 0] = np.where(
        solar_cf[:, 0] <= solar_thr,
        neg_low_dt * 8760.0 / dt_hours,
        neg_high_dt * 8760.0 / dt_hours,
    )
    ...
    for t in range(1, T):
        ...
        solar_latent = solar_latent_season[t] + next_state[:, 1]
        solar_level = _latent_to_physical(
            base.solar_seasonality,
        ...
        lambda_neg[:, t] = np.where(
            solar_level <= solar_thr,
            neg_low_dt * 8760.0 / dt_hours,
            neg_high_dt * 8760.0 / dt_hours,
        )
