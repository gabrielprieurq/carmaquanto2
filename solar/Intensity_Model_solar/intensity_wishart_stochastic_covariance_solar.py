...
def build_full_covariance_proxy(
    ...
    *,
    ...
    frame.columns = ["spot_innovation", "solar_innovation"]

    proxy = pd.DataFrame(
        {
            "s11": frame["spot_innovation"].rolling(window_hours, min_periods=window_hours).var(ddof=1),
            "s22": frame["solar_innovation"].rolling(window_hours, min_periods=window_hours).var(ddof=1),
            "s12": frame["spot_innovation"].rolling(window_hours, min_periods=window_hours).cov(frame["solar_innovation"]),
        }
    ).dropna()
    ...
    sigma_path = np.stack([np.array([[row.s11, row.s12], [row.s12, row.s22]], dtype=float) for row in proxy.itertuples(index=False)])
...
def _evaluate_covariance_fit(proxy: pd.DataFrame, sigma_paths: np.ndarray, eps: float = 1e-10) -> Dict[str, float]:
    ...
    out["rmse_cov_rel_mean"] = float(np.mean([out["rmse_s11_rel"], out["rmse_s22_rel"], out["rmse_s12_rel"]]))
    out["rmse_total"] = float(np.sqrt(np.mean([out["rmse_s11"] ** 2, out["rmse_s22"] ** 2, out["rmse_s12"] ** 2])))
...
def estimate_leverage_vector_wishart(
    ...
    *,
    ...
    innov = innov.iloc[:, :2]
    innov.columns = ["spot_innovation", "solar_innovation"]
    ...
    if len(merged) < 10:
        ...
        diagnostics = pd.DataFrame(
            [
                ...
                {"quantity": "raw_corr_solar_ds22", "value": 0.0},
    ...
    cov_e2_ds22 = float(np.cov(merged["solar_innovation"], merged["ds22"], ddof=1)[0, 1])
    v = np.array(
        [
    ...
    diagnostics = pd.DataFrame(
        [
            ...
            {"quantity": "raw_corr_solar_ds22", "value": float(np.corrcoef(merged["solar_innovation"], merged["ds22"])[0, 1])},
            {"quantity": "cov_spot_ds11", "value": cov_e1_ds11},
            {"quantity": "cov_solar_ds22", "value": cov_e2_ds22},
...
def fit_wishart_full_covariance_model(
    ...
    *,
    ...
    pair = pair.iloc[:, :2]
    pair.columns = ["spot_innovation", "solar_innovation"]
    ...
    pair_std["solar_innovation"] = pair_std["solar_innovation"] / scale_vec[1]
    ...
    hourly_proxy_std, _, _ = build_full_covariance_proxy(pair_std, window_hours=int(selected_fit.window_hours), stride_hours=1, eps=eps)
...
def wishart_parameter_table(calibration: WishartFullCovarianceCalibration) -> pd.DataFrame:
    ...
    rows = [
        ...
        {
            "parameter": "selected_window_hours",
            "value": float(calibration.candidate_fits[calibration.selected_window].window_hours),
            "block": "selection",
        },
    ]
